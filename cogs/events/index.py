...
class Events(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if user_id in self.bot.messages_dump.keys():
            if channel_id not in self.bot.messages_dump[user_id].keys():
                self.bot.messages_dump[user_id][channel_id] = {
                    'message': 1, 'image': 0, 'gif': 0, 'emoji': 0, 'sticker': 0}
            else:
        ...
        if message.stickers:
            self.bot.messages_dump[user_id][channel_id]['sticker'] += len(
                message.stickers)
    ...
    async def on_member_join(self, member):
        ...
        user_in_db = self.bot.db.custom_command(request)[0][0]
        if user_in_db:
            response = self.bot.db.get_value(
                'user_stats', 'roles', 'user_id', member.id)
            if response:
    ...
    async def on_member_remove(self, member):
        ...
        if user_in_db:
            response = self.bot.db.update(
                'user_stats', 'roles', 'user_id', roles, member.id)
        else:
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if after.channel and not before.channel:
            self.bot.voice_users[user_id] = {
                'state': 'connect', 'time': mktime(datetime.now().timetuple())}
        ...
        if not after.channel and before.channel:
            ...
            exists_user = self.bot.db.custom_command(
                f'select exists(select * from web_stat_all_stats where user_id = {user_id} and channel_id = {channel_id});')[0][0]
    ...
    async def on_member_update(self, before, after):
        ...
        result = self.bot.db.custom_command(request)
        print('Update display name for user in database - ',
              result, after.display_name)
    ...
    async def manual_dump(self):
        ...
        for user in exists_user:
            if user_id == user[1] and channel_id == user[2]:
                self.bot.db.custom_command(
                    f'delete from web_stat_all_stats where id = {user[0]}')
                continue
            user_id = user[1]
            channel_id = user[2]
