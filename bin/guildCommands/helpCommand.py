...
async def helpCommand(self, message):
    try:
        ...
        helpMsg = ('`Эта заметка есть в закрепе`\n:snail: **Для создания заявки вам нужно указать следующие параметры;**\n!создать `уровень мира` `ваш уид` `количество слотов свободных в пати` `описание`\n\n' +
        ...
        await message.channel.send(helpMsg, delete_after = 60 * 10)
        await message.delete()
