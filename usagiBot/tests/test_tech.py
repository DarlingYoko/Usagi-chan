...
class TestTechMethods(IsolatedAsyncioTestCase):
    @mock.patch(
        "usagiBot.cogs.Tech.tech_utils.get_user_role", new_callable=mock.AsyncMock
    )
    ...
    async def test_pin_message(self) -> None:
        ...
        self.message.pin.assert_called_with(reason="Pinned by test_author")
        self.ctx.respond.assert_called_with("Message was pinned.", ephemeral=True)

    async def test_go_sleep(self) -> None:
        await self.Tech.go_sleep(self.Tech, self.ctx, 20)
        self.ctx.author.timeout_for.assert_called_with(
            duration=datetime.timedelta(seconds=72000), reason="Timeout for sleep"
        )
        self.ctx.respond.assert_called_with(
            "Good night, see you in 20 hours.", ephemeral=True
        )

    async def test_create_new_unic_role(self) -> None:
        await self.Tech.create_new_unic_role(
            self.Tech, self.ctx, "test_role_name", "111"
        )
        self.ctx.guild.create_role.assert_called_with(
            ...
            hoist=True,
            mentionable=True,
            reason="New role by test_author, id = test_author_id",
        )
    ...
    async def test_delete_unic_role(self) -> None:
        await self.Tech.delete_unic_role(self.Tech, self.ctx, "test_role_name")
        ...
        self.ctx.respond.assert_called_with(
            "Successfully removed your role.", ephemeral=True
        )

    async def test_rename_unic_role(self) -> None:
        await self.Tech.rename_unic_role(
            self.Tech, self.ctx, "test_role_name", "new_name_role"
        )
        self.role.edit.assert_called_with(name="new_name_role")
        self.ctx.respond.assert_called_with(
            "Successfully renamed your role :point_right::skin-tone-1: test_role_mention",
            ephemeral=True,
        )

    async def test_recolor_unic_role(self) -> None:
        await self.Tech.recolor_unic_role(
            self.Tech, self.ctx, "test_role_name", "new_color_role"
        )
        self.role.edit.assert_called_with(color="new_color_role")
        self.ctx.respond.assert_called_with(
            "Successfully recolored your role :point_right::skin-tone-1: test_role_mention",
            ephemeral=True,
        )
