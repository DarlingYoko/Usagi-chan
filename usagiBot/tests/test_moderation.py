...
class TestModerationMethods(IsolatedAsyncioTestCase):
    ...
    def setUp(
        ...
        self.patcher = mock.patch(
            "usagiBot.src.UsagiUtils.check_arg_in_command_tags", new=mock.MagicMock()
        )
    ...
    async def test_set_up_command_create(self) -> None:
        ...
        await self.Moderation.add_config_for_command(
            self.Moderation, self.ctx, self.test_command, self.channel
        )
        ...
        self.ctx.respond.assert_called_with(
            content="Successfully configured channel for command.", ephemeral=True
        )
    ...
    async def test_set_up_command_update(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="Successfully reconfigured channel for command.", ephemeral=True
        )

    async def test_delete_config_for_command(self) -> None:
        self.check_arg_in_command_tags.return_value = True

        await self.Moderation.delete_config_for_command(
            self.Moderation, self.ctx, self.test_command
        )
        self.mock_UsagiConfig.delete.assert_called_with(
            guild_id=self.ctx.guild.id, command_tag=self.test_command
        )
        self.ctx.respond.assert_called_with(
            content="Successfully deleted configure for command.", ephemeral=True
        )
    ...
    async def test_delete_config_for_command_not_in_tags(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This argument does not exist for commands.", ephemeral=True
        )

    async def test_enable_module(self) -> None:
        self.ctx.bot.guild_cogs_settings = {}
        await self.Moderation.enable_module(self.Moderation, self.ctx, "Fun")
        ...
        self.ctx.respond.assert_called_with(
            content="The `Fun` module has been enabled.", ephemeral=True
        )
        self.assertEqual(
            self.ctx.bot.guild_cogs_settings, {"test_guild_id": {"Fun": True}}
        )

    async def test_enable_module_isnt_available(self) -> None:
        self.ctx.bot.guild_cogs_settings = {}
        await self.Moderation.enable_module(self.Moderation, self.ctx, "Moderation")

        self.ctx.respond.assert_called_with(
            content="This module isn't available.", ephemeral=True
        )
    ...
    async def test_enable_module_is_already_enabled(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This module already enabled.", ephemeral=True
        )

    async def test_disable_module(self) -> None:
        self.ctx.bot.guild_cogs_settings = {"test_guild_id": {"Fun": True}}
        await self.Moderation.disable_module(self.Moderation, self.ctx, "Fun")
        ...
        self.ctx.respond.assert_called_with(
            content="The `Fun` module has been disabled.", ephemeral=True
        )
        self.assertEqual(self.ctx.bot.guild_cogs_settings, {})

    async def test_disable_module_isnt_available(self) -> None:
        self.ctx.bot.guild_cogs_settings = {}
        await self.Moderation.disable_module(self.Moderation, self.ctx, "Moderation")
    ...
    async def test_disable_module_is_already_enabled(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This module isn't enabled.", ephemeral=True
        )

    async def test_add_new_moder_role(self) -> None:
        self.ctx.bot.moder_roles = {}

        await self.Moderation.add_new_moder_role(
            self.Moderation, self.ctx, self.moder_role
        )
        ...
        self.ctx.respond.assert_called_with(
            content="The `test_moder_role_mention` role has been added.", ephemeral=True
        )
        self.assertEqual(
    ...
    async def test_add_new_moder_role_already_exist(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This role already added.", ephemeral=True
        )

    async def test_remove_moder_role(self) -> None:
        self.ctx.bot.moder_roles = {"test_guild_id": ["test_moder_role_id"]}

        await self.Moderation.remove_moder_role(
            self.Moderation, self.ctx, self.moder_role
        )
    ...
    async def test_remove_moder_role_not_exist(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This role isn't moderation.", ephemeral=True
        )

    async def test_show_moder_roles(self) -> None:
        self.ctx.bot.moder_roles = {"test_guild_id": ["test_moder_role_id"]}

        await self.Moderation.show_moder_roles(self.Moderation, self.ctx)
    ...
    async def test_show_moder_roles_no_roles(self) -> None:
        ...
        self.ctx.respond.assert_called_with(
            content="This guild doesn't have any Moderation roles.", ephemeral=True
        )
