...
class TestCheckMethods(IsolatedAsyncioTestCase):
    ...
    async def test_check_is_already_set_up(self) -> None:
        ...
        try:
            respone = await self.UsagiChecks.check_is_already_set_up().predicate(
                self.ctx
            )
        except UsagiNotSetUpError:
            self.fail(
                "check_is_already_set_up() raised UsagiNotSetUpError unexpectedly!"
            )

        self.mock_UsagiConfig.get.assert_called_with(
            guild_id="test_guild_id", command_tag="test_command_tag"
        )
    ...
    async def test_check_correct_channel_command(self) -> None:
        ...
        try:
            respone = await self.UsagiChecks.check_correct_channel_command().predicate(
                self.ctx
            )
        except UsagiNotSetUpError:
            self.fail(
                "check_correct_channel_command() raised UsagiNotSetUpError unexpectedly!"
            )
        except UsagiCallFromWrongChannelError:
            self.fail(
                "check_correct_channel_command() raised UsagiCallFromWrongChannelError unexpectedly!"
            )
    ...
    async def test_check_cog_whitelist(self) -> None:
        self.ctx.bot.guild_cogs_settings = {"test_guild_id": {"test_cog_name": True}}
    ...
    async def test_check_cog_whitelist_not_in(self) -> None:
        self.ctx.bot.guild_cogs_settings = {"test_guild_id": {}}
        cog = mock.MagicMock()
        respone = self.UsagiChecks.check_cog_whitelist(cog, self.ctx)
    ...
    async def test_check_member_is_moder_have_moder_role(self) -> None:
        ...
        try:
            ...
            self.fail(
                "check_member_is_moder() raised UsagiCallFromNotModerError unexpectedly!"
            )
