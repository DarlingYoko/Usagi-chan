...
class TestTwitch(IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls, mock_first, mock_engine, mock_UsagiConfig, mock_UsagiTwitchNotify) -> None:
    ...
    async def test_twitch_notify_loop(self) -> None:
        ...
        self.mock_UsagiConfig.get.return_value = mock.MagicMock(generic_id="test_generic_id_channel_to_notify")
        stream = mock.MagicMock()
        self.mock_first.return_value = stream
        ...
        self.mock_UsagiConfig.get.assert_called_with(guild_id="test_guild_id_1", command_tag="twitch_notify")
        self.Twitch.bot.fetch_channel.assert_called_with("test_generic_id_channel_to_notify")

        notify_channel.send.assert_has_calls([
            mock.call(f"test_twitch_username_1 start stream, <@test_user_id_1>"),
            mock.call(f"test_twitch_username_2 start stream, <@test_user_id_1> <@test_user_id_2>"),
            mock.call(f"test_twitch_username_3 start stream, <@test_user_id_2>")
        ], any_order=True)
    ...
    async def test_follow_streamer(self) -> None:
        ...
        self.ctx.respond.assert_called_with(f"Followed you to yoko_0", ephemeral=True)
    ...
    async def test_follow_streamer_fake_streamer(self) -> None:
        ...
        self.ctx.respond.assert_called_with("There isn't streamer with that nickname!", ephemeral=True)

    async def test_follow_streamer_already_followed(self) -> None:
        self.mock_UsagiTwitchNotify.get.return_value = mock.MagicMock()
        await self.Twitch.follow_streamer.callback(self.Twitch, self.ctx, "uselessmouth")
        ...
        self.ctx.respond.assert_called_with(f"You are already followed to this streamer!", ephemeral=True)

    async def test_unfollow_streamer(self) -> None:
        self.mock_UsagiTwitchNotify.get.return_value = mock.MagicMock()
        await self.Twitch.unfollow_streamer.callback(self.Twitch, self.ctx, "streamer_1")
        self.mock_UsagiTwitchNotify.get.assert_called_with(
            guild_id="test_guild_id",
            user_id="test_user_id",
        ...
        self.ctx.respond.assert_called_with(f"Unfollowed you from streamer_1", ephemeral=True)

    async def test_unfollow_streamer_not_followed(self) -> None:
        self.mock_UsagiTwitchNotify.get.return_value = None
        await self.Twitch.unfollow_streamer.callback(self.Twitch, self.ctx, "streamer_2")
        self.mock_UsagiTwitchNotify.get.assert_called_with(
            guild_id="test_guild_id",
            user_id="test_user_id",
            twitch_username="streamer_2",
        )
        self.ctx.respond.assert_called_with(f"You are not followed to this streamer", ephemeral=True)
    ...
    async def test_show_user_follows(self) -> None:
        ...
        response = "Your followed streamers:\n - streamer_1\n - streamer_2\n - streamer_3\n"
    ...
    async def test_show_user_follows_no_streamers(self) -> None:
        ...
        self.ctx.respond.assert_called_with("You are not followed to any streamer", ephemeral=True)
    ...
    async def test_show_all_follows_no(self) -> None:
        ...
        self.ctx.respond.assert_called_with("Your guild not followed to any streamer", ephemeral=True)
