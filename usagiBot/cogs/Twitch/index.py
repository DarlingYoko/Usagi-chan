...
from usagiBot.cogs.Twitch.twitch_utils import twitch_auth
...
class Twitch(commands.Cog):
    ...
    async def twitch_notify_loop(self):
        ...
        for stream in all_streams:
            config = await UsagiConfig.get(guild_id=stream.guild_id, command_tag="twitch_notify")
            if not config:
                continue
            channel = await self.bot.fetch_channel(config.generic_id)

            guild_notify = twitch_notify.setdefault(stream.guild_id, {"channel_to_notify": channel})
            streamers_notify = guild_notify.setdefault("streamers", {})
            username_notify = streamers_notify.setdefault(stream.twitch_username,
                                                          {"followers": [], "started_at": stream.started_at})
            username_notify["followers"].append(stream.user_id)
            username_notify["started_at"] = min(username_notify["started_at"], stream.started_at)
        ...
        for guild_notify, data in twitch_notify.items():
            ...
            for streamer, info in data["streamers"].items():
                ...
                if stream.started_at.replace(tzinfo=None) == info["started_at"]:
                    continue
                followers = " ".join(list(map(lambda x: f"<@{x}>", info["followers"])))
                message = f"{streamer} start stream, {followers}"
                await channel.send(message)

                update_streamer.setdefault(streamer, stream.started_at)

        for streamer, new_time in update_streamer.items():
            await UsagiTwitchNotify.update_all(
                {"twitch_username": streamer},
                {"started_at": new_time.replace(tzinfo=None)}
            )
    ...
    async def follow_streamer(
        ...
        if user is None:
            await ctx.respond("There isn't streamer with that nickname!", ephemeral=True)
            return
        ...
        if streamer is not None:
            await ctx.respond("You are already followed to this streamer!", ephemeral=True)
            return
    ...
    @twitch_notify.command(name="show_all", description="Show all streamers from guild.")
