...
error_messages = {
    "ERROR: Sign in to confirm your age\nThis video may be inappropriate for some users.": loc["error"][
        "age_restriction"],
...
class Song:
    __slots__ = ('source', 'requester', 'starttime', 'pause_time', 'pause_duration', 'paused', 'isFile', 'isDirectLink',
                 'time_invoke_seek')
    ...
    def create_embed(self, status: str):
        ...
        embed = (discord.Embed(title=loc["song_embed"]["title"],
                 ...
                 .add_field(name=loc["song_embed"]["video_length"], value=(
            self.source.duration if status == "play" else YTDLSource.parse_duration_raw(
                int(time.time() - self.starttime - self.pause_duration)) + "/" + self.source.duration_raw))
                 .add_field(name=loc["song_embed"]["video_player"], value=self.requester.mention))
        # If it is not a file, it is a youtube video
        if not self.isFile:
            embed.add_field(name=loc["song_embed"]["uploader"],
                            value=loc["song_embed"]["uploader_body"].format(self.source.uploader,
                                                                            self.source.uploader_url))
            embed.add_field(name=loc["song_embed"]["video_url"],
...
class VoiceState:
    ...
    async def seek(self, seconds, isLocal, isDirectLink):
        ...
        if isLocal or isDirectLink:
            self.current = await self.create_song_source(self._ctx, self.current.source.url,
                                                         ...
                                                         requester=self.current.source.requester, seek=seconds,
    ...
    async def update_volume(self):
        ...
        while self.is_playing:
            ...
            if not isinstance(self.current, dict) and not isinstance(self.current,
                                                                     str) and self.current and self.current.source.volume != self._volume:
                self.current.source.volume = self._volume
    ...
    async def create_song_source(self, ctx, url, title=None, requester=None, seek=None, duration=None):
        ...
        if "local@" in url:
            ...
            try:
                ...
                duration = str(int(float(subprocess.check_output(
                    f"ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 \"{url}\"",
                    shell=True).decode("ascii").replace("\r", "").replace("\n", ""))))
            except:
                return "error"
            # Return the song object with ffmpeg
            if seek is not None:
                return Song(FFMPEGSource(ctx, discord.FFmpegPCMAudio(url,
                                                                     before_options="-ss " + YTDLSource.parse_duration_raw(
                                                                         seek)),
                                         data={'duration': duration, 'title': title, 'url': "local@" + url,
                                               'requester': requester}, seek=seek), True)
            else:
                return Song(FFMPEGSource(ctx, discord.FFmpegPCMAudio(url),
                                         data={'duration': duration, 'title': title, 'url': "local@" + url,
            ...
            try:
                result = Song(
                    await YTDLSource.create_source(ctx, url, loop=self.bot.loop, requester=requester, seek=seek))
            ...
            try:
                result = Song(
                    await FFMPEGSource.create_source(ctx, url, loop=self.bot.loop, requester=requester, seek=seek),
                    True, True)
            except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                if self.voice and len(self.voice.channel.members) == 1:
                    ...
                    if (
                            len(self.voice.channel.members) == 1 and self.me in self.voice.channel.members) or self.me not in self.voice.channel.members:
                    ...
                    try:
                        async with timeout(120):  # 2 minutes
                            ...
                            try:
                                ...
                                domain = ""
                            if "youtube" in domain or "youtu.be" in domain:
                                self.current = await self.create_song_source(self._ctx, self.next_song["url"],
                            ...
                            if self.current != "error":
                                ...
                                if self.loopqueue:
                                    await self.songs.put(
                                        {"url": self.current.source.url, "title": self.current.source.title,
...
class SearchMenu(discord.ui.Select):
    def __init__(self, bot, options_raw, cog, ctx):
        ...
        options = [discord.SelectOption(label=data["title"],
                                        description=loc["search_menu"]["video_length"].format(data['duration']),
                                        value=str(data["index"]), emoji=reaction_list[data["index"]]) for data in
                   options_raw]
        options.append(
            discord.SelectOption(label=loc["search_menu"]["cancel"], description=loc["search_menu"]["cancel_desc"],
                                 value="11", emoji="❌"))
    ...
    async def respond(self, message_reply, message: str = None, color=discord.Embed.Empty, embed=None,
                      reply: bool = True, view=None):
        if useEmbed:
            if embed is None:
    ...
    async def callback(self, interaction):
        if int(self.values[0]) == 11:
            ...
            return await interaction.message.edit(
                embed=discord.Embed(title=loc["search_menu"]["cancelled_search"], description=None,
                                    color=discord.Color.green()), view=None)
        # Edit the message to reduce its size
        await interaction.message.edit(
            embed=discord.Embed(title=loc["search_menu"]["result"], description=self.data[int(self.values[0])]["title"],
                                color=discord.Color.green()), view=None)
        ...
        if not ctx.debug["debug"]:
            ...
            if not ctx.author.voice or not ctx.author.voice.channel or (
                    ctx.voice_state.voice and ctx.author.voice.channel != ctx.voice_state.voice.channel):
                return await self.respond(interaction.message, loc["messages"]["enter_before_play"], color=0xff0000)
            ...
            if "/playlist?" in search:
                ...
                for songs, entry in enumerate(playlist):
                    ...
                    await ctx.voice_state.songs.put(
                        {"url": entry["url"], "title": entry["title"], "user": ctx.author, "duration": duration})
                await self.respond(interaction.message, loc["messages"]["added_multiple_song"].format(songs + 1),
                                   color=0x1eff00)
            else:
                # Just a single song
                try:
                    ...
                    return await self.respond(interaction.message, loc["messages"]["error"].format(message),
                                              color=0xff0000)
                if "entries" in data:
                ...
                try:
                    ...
                    duration = 0
                await ctx.voice_state.songs.put(
                    {"url": data["webpage_url"], "title": data["title"], "user": ctx.author, "duration": duration})
                await self.respond(interaction.message, loc["messages"]["added_song"].format(data['title']),
                                   color=0x1eff00)
...
class SearchView(discord.ui.View):
    ...
    async def on_timeout(self):
        if not self.children[0].completed:
            await self.message.edit(embed=discord.Embed(title=loc["search_menu"]["search_end"], color=0xff0000),
                                    view=None)
...
class PlayerControlView(discord.ui.View):
    def __init__(self, bot, voice_state):
        ...
        self.children[4].label = loc["loop"]["disable_loopqueue"] if self.voice_state.loopqueue else loc["loop"][
            "enable_loopqueue"]

    @discord.ui.button(label=loc["btn_label"]["pause"], style=discord.ButtonStyle.primary, custom_id="0", emoji="⏸",
                       disabled=False)
    ...
    @discord.ui.button(label=loc["btn_label"]["skip"], style=discord.ButtonStyle.primary, custom_id="1", emoji="⏭",
                       disabled=False)
    ...
    @discord.ui.button(label=loc["btn_label"]["stop"], style=discord.ButtonStyle.primary, custom_id="2", emoji="⏹",
                       disabled=False)
    ...
    @discord.ui.button(label=loc["btn_label"]["loop"], style=discord.ButtonStyle.primary, custom_id="3", emoji="🔂",
                       disabled=False)
    ...
    @discord.ui.button(label=loc["btn_label"]["loopqueue"], style=discord.ButtonStyle.primary, custom_id="4", emoji="🔂",
                       disabled=False)
        ...
        try:
            if self.voice_state.loopqueue:
                await self.voice_state.songs.put(
                    {"url": self.voice_state.current.source.url, "title": self.voice_state.current.source.title,
    ...
    @discord.ui.button(label=loc["btn_label"]["queue"], style=discord.ButtonStyle.primary, custom_id="5", emoji="📜",
                       disabled=False)
    async def queue(self, button, interaction):
        ...
        if len(self.voice_state.songs) == 0 and self.voice_state.current is None:
            if useEmbed:
                return interaction.response.send_message(
                    embed=discord.Embed(title=loc["messages"]["empty_queue"], color=0xff0000))
        ...
        return await interaction.response.send_message(
            embed=Music.queue_embed(self.voice_state.songs, page, loc["queue_embed"]["title"],
                                    loc["queue_embed"]["body"].format(self.voice_state.current.source.title,
                                                                      self.voice_state.current.source.url,
                                                                      YTDLSource.parse_duration(
                                                                          self.voice_state.current.source.duration_int - int(
                                                                              time.time() - self.voice_state.current.starttime - self.voice_state.current.pause_duration))),
                                    "url"))
...
class Music(commands.Cog):
    ...
    def queue_embed(data, page, header, description, song_id):
        ...
        embed = (discord.Embed(
            ...
            description=description)
                 .add_field(
            name=loc["queue_embed"]["embed_body"].format(len(data), YTDLSource.parse_duration(getTotalDuration(data))),
            value=queue)
    ...
    async def respond(self, ctx, message: str = None, color=discord.Embed.Empty, embed=None, reply: bool = True,
                      view=None):
        if useEmbed:
            if embed is None:
                embed = discord.Embed(title=message, color=color)
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if not (
                str(error) == "Application Command raised an exception: AttributeError: 'NoneType' object has no attribute 'is_finished'"):
        ...
        if hasattr(ctx, "voice_state") and ctx.voice_state:
            if ctx.voice_state.debug["debug_log"]:
                await ctx.send("Debug file",
                               file=discord.File(io.BytesIO(base64.b64encode(formatted_error.encode("utf-8"))),
                                                 f"{ctx.guild.id}_error.txt"))

    @music.command(name='join', invoke_without_subcommand=True, description=loc["descriptions"]["join"])
    ...
    async def summon(self, ctx, channel=None):
        ...
        if ctx.voice_state.voice:
            ...
            msg = await self.respond(ctx.ctx,
                                     loc["messages"]["joined_summon"].format(ctx.voice_state.voice.channel.name,
                                                                             destination.name), color=0x1eff00)
    ...
    async def queue(self, ctx, page=None):
        ...
        while ctx.voice_state.current is None or isinstance(ctx.voice_state.current, dict):
            await asyncio.sleep(1)
        return await self.respond(ctx.ctx,
                                  embed=self.queue_embed(ctx.voice_state.songs, page, loc["queue_embed"]["title"],
    ...
    async def loop(self, ctx):
        ...
        await self.respond(ctx.ctx, (
            loc["messages"]["enable_loop"] if ctx.voice_state.loop else loc["messages"]["disable_loop"]),
                           color=0x1eff00)
    ...
    async def play(self, ctx, link):
        ...
        if not ctx.debug["debug"]:
            ...
            if not ctx.author.voice or not ctx.author.voice.channel or (
                    ctx.voice_state.voice and ctx.author.voice.channel != ctx.voice_state.voice.channel):
                return await ctx.respond(loc["messages"]["enter_before_play"], ephemeral=True)

            if ctx.voice_client:
                if ctx.voice_client.channel != ctx.author.voice.channel:
                    return await ctx.respond(loc["messages"]["already_in_voice"], ephemeral=True)
        ...
        try:
            ...
            if "/playlist?" in link and ("youtu.be" in link or "youtube.com" in link):
                partial = functools.partial(YTDLSource.ytdl_playlist.extract_info, link, download=False)
                data = await loop.run_in_executor(None, partial)
                if data is None:
                    return await ctx.respond(loc["not_found"].format(link), ephemeral=True)
                entries = data["entries"]
                playlist = []
                for pos, song in enumerate(entries):
                    # Youtube only, guess no one would play other than Youtube
                ...
                for songs, entry in enumerate(playlist):
                    ...
                    await ctx.voice_state.songs.put(
                        {"url": entry["url"], "title": entry["title"], "user": ctx.author, "duration": duration})
                await ctx.respond(loc["messages"]["added_multiple_song"].format(songs + 1), ephemeral=True)
            else:
                # Just a single song
                try:
                    domain = link.split("/")[2]
                ...
                if "youtube" not in domain and "youtu.be" not in domain:
                    ...
                    try:
                        partial = functools.partial(YTDLSource.ytdl.extract_info, link, download=False)
                        data = await loop.run_in_executor(None, partial)
                        if "entries" in data:
                            if len(data["entries"]) > 0:
                                data = data["entries"][0]
                        ...
                        title = data["title"]
                        if "duration" in data:
                            await ctx.voice_state.songs.put({"url": link, "title": title, "user": ctx.author,
                        ...
                        return await ctx.respond(loc["messages"]["cannot_add_invalid_file"], ephemeral=True)
                    await ctx.respond(loc["messages"]["added_song"].format(title.replace("_", "\\_")),
                                       ephemeral=True)
                else:
                    try:
                        partial = functools.partial(YTDLSource.ytdl.extract_info, link, download=False)
                        ...
                        # Get the error message from dictionary, if it doesn't exist in dict, return the original error message
                        message = error_messages.get(str(e), str(e))
                        return await ctx.respond(loc["messages"]["error"].format(message), ephemeral=True)
                    if "entries" in data:
                        if len(data["entries"]) > 0:
                            data = data["entries"][0]
                        else:
                    ...
                    await ctx.voice_state.songs.put(
                        {"url": data["webpage_url"], "title": data["title"], "user": ctx.author, "duration": duration})
                    await ctx.respond(loc["messages"]["added_song"].format(data['title']), ephemeral=True)
            ctx.voice_state.stopped = False
        except YTDLError as e:
    ...
    async def search(self, ctx, keyword=None):
        ...
        for count, entry in enumerate(result):
            embed.add_field(name=f'{count + 1}. {entry["title"]}',
                            value=loc["messages"]["search_row"].format(entry["url"], entry["duration"]), inline=False)
    ...
    async def loopqueue(self, ctx):
        ...
        try:
            if ctx.voice_state.loopqueue:
                await ctx.voice_state.songs.put(
                    {"url": ctx.voice_state.current.source.url, "title": ctx.voice_state.current.source.title,
                     "user": ctx.voice_state.current.source.requester,
                     "duration": ctx.voice_state.current.source.duration_int})
        except:
            pass
        await self.respond(ctx.ctx, (
            loc["messages"]["enable_loopqueue"] if ctx.voice_state.loopqueue else loc["messages"]["disable_loopqueue"]),
                           color=0x1eff00)
            ...
            try:
                ...
                if regexp.match(seconds).group() != "":
                    ...
                    if ctx.voice_state.current.paused:
                        ...
                        ctx.voice_state.current.pause_time = time.time()
                    seconds = int(
                        time.time() - ctx.voice_state.current.starttime - ctx.voice_state.current.pause_duration) + int(
                        seconds[1:])
                elif seconds[0] == "-":
                    ...
                    if ctx.voice_state.current.paused:
                        ...
                        ctx.voice_state.current.pause_time = time.time()
                    seconds = max((
                                              int(time.time() - ctx.voice_state.current.starttime - ctx.voice_state.current.pause_duration) - int(
                                          seconds[1:])), 0)
                else:
                    seconds = int(seconds)
            except:
    ...
    async def musicversion(self, ctx):
        await self.respond(ctx.ctx, embed=discord.Embed(title=loc["cog_name"]).add_field(name=loc["author"],
                                                                                         value="<@127312771888054272>").add_field(
            name=loc["cog_github"], value=loc["cog_github_link"]).add_field(name=loc["cog_language"], value=language))
