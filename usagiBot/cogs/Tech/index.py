...
class Tech(commands.Cog):
    ...
    @commands.message_command(
        name="Pin message",
        name_localizations={"ru": "Закрепить сообщение"},
    )
    ...
    async def go_sleep(
        ...
        await ctx.respond(
            _("Good night, see you in").format(hours=hours), ephemeral=True
        )
    ...
    @discord.commands.option(
        ...
        description_localizations={
            "ru": "Цвет твоей роли в шестнадцатеричной системе."
        },
    )
    async def create_new_unic_role(self, ctx, name: str, color: ColorConverter) -> None:
        ...
        await ctx.respond(
            _("Created a new role for you").format(mention=role.mention), ephemeral=True
        )
    ...
    async def rename_unic_role(self, ctx, role, new_name: str) -> None:
        role = await get_user_role(ctx, role)
        if role:
    ...
    async def recolor_unic_role(self, ctx, role, new_color: ColorConverter) -> None:
        role = await get_user_role(ctx, role)
        if role:
